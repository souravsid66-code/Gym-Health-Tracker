"""This is a simple Flask application that demonstrates basic 
...
def get_transactions():
    """List all transactions and calculate the total balance."""
    # Saari amounts ka total nikalna
    total = sum(t['amount'] for t in transactions)
    # Total ko bhi template mein bhejna
...
def edit_transaction(transaction_id):
    """Handle the editing of an existing transaction."""
    # Check if the request method is POST (form submission)
    if request.method == 'POST':
    ...
    # If the request method is GET, find the transaction with the matching ID and
...
def delete_transaction(transaction_id):
    """Handle the deletion of an existing transaction."""
...
def search_transactions():
    """Handle searching transactions by amount range."""
    if request.method == 'POST':
        # User ne jo range di hai use pakadna
        min_amount = float(request.form['min_amount'])
        max_amount = float(request.form['max_amount'])

        # List Comprehension se filter karna
        filtered_transactions = [t for t in transactions if min_amount <= t['amount'] <= max_amount]

        # Filtered list ko dikhana
        return render_template("transactions.html", transactions=filtered_transactions)

    # Agar sirf page khola hai (GET), toh search form dikhao
